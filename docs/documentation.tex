...
The Alert Manager orchestrates rule filtering, 60-second deduplication (keyed by \texttt{src\_ip}, \texttt{dst\_ip}, \texttt{attack\_type}), severity scoring, SQLite persistence (WAL mode), Prometheus metric incrementing, and WebSocket streaming.
...
\begin{enumerate}
    ...
    \item \textbf{Prometheus Alertmanager Integration}: Deliver alert notifications via Prometheus Alertmanager routing, supporting enterprise email and webhook delivery channels while keeping the IDS detection loop decoupled.
